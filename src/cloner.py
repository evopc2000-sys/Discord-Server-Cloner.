    ...
    async def create_categories(self):
        for category in self.input_guild.categories:
            ...
            for key, value in category.overwrites.items():
                role = discord.utils.get(self.output_guild.roles, name=key.name) if hasattr(key, 'name') else None
                if role:
            ...
            await new_category.edit(position=int(category.position))
            ...
            if channel.category_id is not None:
                ...
                if new_category_id:
                    ...
                    new_channel = await new_category.create_text_channel(
                        ...
                        slowmode_delay=channel.slowmode_delay, nsfw=channel.is_nsfw(), overwrites=overwrites_to
                    )
                    log(blue+'[ServerCloner]'+r, f'Created channel {cyan+str(new_channel.id)+r}.')
            else:
                new_channel = await self.output_guild.create_text_channel(
                    name=channel.name, topic=channel.topic, position=channel.position,
                    slowmode_delay=channel.slowmode_delay, nsfw=channel.is_nsfw(), overwrites=overwrites_to
                )
                log(blue+'[ServerCloner]'+r, f'Created channel {cyan+str(new_channel.id)+r}.')
            ...
            if channel.category_id is not None:
                ...
                if new_category_id:
                    ...
                    new_channel = await new_category.create_voice_channel(
                        name=channel.name, position=channel.position, user_limit=channel.user_limit, overwrites=overwrites_to
                    )
                    log(blue+'[ServerCloner]'+r, f'Created channel {cyan+str(new_channel.id)+r}.')
            else:
                new_channel = await self.output_guild.create_voice_channel(
                    name=channel.name, position=channel.position, user_limit=channel.user_limit, overwrites=overwrites_to
                )
                log(blue+'[ServerCloner]'+r, f'Created channel {cyan+str(new_channel.id)+r}.')
